...
static void hmac_sha512(const uint8_t* key, size_t key_len,
    ...
    if (!key || !mac) return;  // NULL 포인터 검사
    if (mac_len == 0 || mac_len > SHA512_DIGEST_SIZE) return;  // MAC 길이 검증
...
aes_status_t aes_hmac_decrypt_and_verify(
    ...
    // 전체 SHA-512 해시를 계산 (mac_len과 무관하게 항상 64바이트 계산)
    uint8_t computed_mac[SHA512_DIGEST_SIZE];
    hmac_sha512(mac_key, mac_key_len, mac_input ? mac_input : (const uint8_t*)"", mac_input_len, computed_mac, SHA512_DIGEST_SIZE);

    if (mac_input) {
        free(mac_input);
    }

    // 상수 시간 비교 (mac_len만큼만 비교)
    uint8_t diff = 0;
