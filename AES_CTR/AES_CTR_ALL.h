...
#define AES_MAX_RK_WORDS   60u  // 최대 워드 수: AES-256이 가장 많이 필요하므로 60워드
    ...
    /* ===== 2) 코어 블록 암호화 함수 ===== */
    // AES 블록 암호화: 16바이트 평문(pt) 1블록을 16바이트 암호문(ct)으로 변환
    ...
    // counter_block: 16바이트 카운터 블록 (in-place 수정됨)
    // 맨 뒤에서부터 counter_len 바이트(1~16)만큼 big-endian 방식으로 1 증가
    ...
    // src: 입력 데이터 포인터, len: 처리할 데이터 길이 (바이트 단위), dst: 출력 데이터 포인터
