// AES_CTR_ALL.c — AES-128/192/256 및 CTR 모드 구현 (8x32 테이블 사용)
...
// T-tables for 8x32 optimization (SubBytes + ShiftRows + MixColumns combined)
...
// Helper: 128비트 바이트 배열을 4개의 32비트 워드로 변환
static void bytes_128b_to_words_32b(const uint8_t in[16], uint32_t words[4]) {
    for (int i = 0; i < 4; i++) {
        words[i] = mkw(&in[4 * i]);
    }
}

// Helper: 4개의 32비트 워드를 4x4 바이트 상태로 변환
static void words_32b_to_state_4x4_8b(const uint32_t words[4], uint8_t state[4][4]) {
    for (int i = 0; i < 4; i++) {
        state[0][i] = (uint8_t)((words[i] >> 24) & 0xff);
        state[1][i] = (uint8_t)((words[i] >> 16) & 0xff);
        state[2][i] = (uint8_t)((words[i] >> 8) & 0xff);
        state[3][i] = (uint8_t)(words[i] & 0xff);
    }
}
...
aes_status_t aes_init_ctx_key_bytes(aes_ctx_t* ctx, const uint8_t* key, size_t key_len) {
    ...
    if (key_len == 16) {
        ...
        return AES_ERR_KEYLEN;
    }
    
    // 키 스케줄링 (AES-128/192/256 공통 로직)
    size_t nk = key_len / 4;  // 키 워드 수
    size_t nr = ctx->rounds;   // 라운드 수
    ...
    for (size_t i = nk; i < 4 * (nr + 1); i++) {
        uint32_t temp = ctx->rk_enc[i - 1];
        if (i % nk == 0) {
            temp = subw(rotl8(temp)) ^ ((uint32_t)RCON[(i / nk) - 1] << 24);
        } else if (nk > 6 && (i % nk == 4)) {
            temp = subw(temp);
        }
        ctx->rk_enc[i] = ctx->rk_enc[i - nk] ^ temp;
    }
    
    return AES_OK; 
}

/* ===== 2) 코어 블록 암호화 함수 (AES-128/192/256 공용, 8x32 테이블 사용) ===== */
aes_status_t aes_encrypt_block_128b_in_128b_out(const aes_ctx_t* ctx,
    ...
    uint8_t s[4][4]; // 4x4 바이트 상태 행렬
    bytes_128b_to_state_4x4_8b(pt, s); // 바이트 배열을 4x4 상태로 변환

    // AddRoundKey(라운드 0): 첫 번째 라운드 키를 상태에 XOR
    for (int j = 0; j < 4; j++) {
        uint32_t w = ctx->rk_enc[j];
        s[0][j] ^= (uint8_t)((w >> 24) & 0xff);
        s[1][j] ^= (uint8_t)((w >> 16) & 0xff);
        s[2][j] ^= (uint8_t)((w >> 8) & 0xff);
        s[3][j] ^= (uint8_t)(w & 0xff);
    }

    // 라운드 1부터 (Nr-1)까지 수행: T-table을 사용한 표준 라운드
    for (int r = 1; r < (int)ctx->rounds; r++) {
        // T-table을 사용한 SubBytes, ShiftRows, MixColumns (각 컬럼에 대해)
        uint32_t new_col[4];
        for (int j = 0; j < 4; j++) {
            // ShiftRows가 적용된 상태에서 T-table 조회
            // 컬럼 j: s[0][j], s[1][(j+1)%4], s[2][(j+2)%4], s[3][(j+3)%4]
            new_col[j] = T0[s[0][j]] ^
                         T1[s[1][(j + 1) % 4]] ^
                         T2[s[2][(j + 2) % 4]] ^
                         T3[s[3][(j + 3) % 4]];
        ...
        // 새로운 컬럼 값을 상태에 저장
        for (int j = 0; j < 4; j++) {
            s[0][j] = (uint8_t)((new_col[j] >> 24) & 0xff);
            s[1][j] = (uint8_t)((new_col[j] >> 16) & 0xff);
            s[2][j] = (uint8_t)((new_col[j] >> 8) & 0xff);
            s[3][j] = (uint8_t)(new_col[j] & 0xff);
        }
        
        // AddRoundKey: 이번 라운드의 라운드 키를 상태에 XOR
        for (int j = 0; j < 4; j++) {
            uint32_t w = ctx->rk_enc[4 * r + j];
            s[0][j] ^= (uint8_t)((w >> 24) & 0xff);
            s[1][j] ^= (uint8_t)((w >> 16) & 0xff);
            s[2][j] ^= (uint8_t)((w >> 8) & 0xff);
            s[3][j] ^= (uint8_t)(w & 0xff);
    ...
    for (int j = 0; j < 4; j++) {
        uint32_t w = ctx->rk_enc[4 * ctx->rounds + j];
        s[0][j] ^= (uint8_t)((w >> 24) & 0xff);
        s[1][j] ^= (uint8_t)((w >> 16) & 0xff);
        s[2][j] ^= (uint8_t)((w >> 8) & 0xff);
        s[3][j] ^= (uint8_t)(w & 0xff);
    ...
    state_4x4_8b_to_bytes_128b(s, ct);
    return AES_OK;
}
...
aes_status_t aes_ctr_xor_stream_bytes_in_bytes_out(const aes_ctx_t* ctx,
    ...
    size_t done = 0;
    while (done < len) {  
        aes_status_t st = aes_encrypt_block_128b_in_128b_out(ctx, counter_block, ks);
        ...
        ctr_increment_128b_inout(counter_block, counter_len);
    }
    return AES_OK;  
}
